...
inline void Timer::printElapsed() {
    if (_running) {
        gettimeofday(_time2,NULL);
        std::cout << "Time elapsed : " << _cumul + static_cast<double>((_time2->tv_sec -
                    _time1->tv_sec)*1000000 + _time2->tv_usec-_time1->tv_usec)/1000000.0 << std::endl;
    } else {
        std::cout << "Time elapsed : " << _cumul << std::endl;
    }
};
